...
def test_start_instances(awstools):
    with mock_ec2():
        ...
        assert awstools.ec2_client.describe_instance_attribute(
            Attribute='userData',
            InstanceId=queue.instance_id)['UserData']['Value'] == base64.b64encode(
                awstools.user_data_scripts['queue']).decode('ascii')
        assert awstools.ec2_client.describe_instance_attribute(
            Attribute='userData',
            InstanceId=workers[0].instance_id)['UserData']['Value'] == base64.b64encode(
                awstools.user_data_scripts['worker']).decode('ascii')
        assert awstools.ec2_client.describe_instance_attribute(
            Attribute='userData',
            InstanceId=workers[1].instance_id)['UserData']['Value'] == base64.b64encode(
                awstools.user_data_scripts['worker']).decode('ascii')
