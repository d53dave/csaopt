...
class AWSTools(InstanceManager):
    ...
    This is a context manager and creates required instance on `__enter__()`, disposing of the managed instances in
    `__exit__()`.
    ...
    def __init__(self, config: ConfigTree, internal_conf: ConfigTree) -> None:
        ...
        self.broker_ami = config.get(queue_ami_key, internal_conf[queue_ami_key])
    ...
    def _provision_instances(self, timeout_ms: int, count: int = 2, **kwargs: Dict[str, Any]) -> Tuple[Any, Any]:
        ...
        message_queue = self.ec2_resource.create_instances(
            ImageId=imageId,
            MinCount=1,
            MaxCount=1,
    ...
    def __map_ec2_instance(self, instance: Any, is_broker: bool = False, **kwargs: Dict[str, Any]) -> Instance:
    ...
    def get_running_instances(self) -> Tuple[Instance, List[Instance]]:
        """Returns a tuple of broker, list[worker]"""
        return (
                ...
                IpPermissions=[  # TODO: 80 and 22 are not needed here. Rather, these should be the zmq ports
                    {
                        ...
                        'ToPort': 80,
                        'IpRanges': [{
                            'CidrIp': '{}/0'.format(self.own_external_ip)
                        }]
                    }, {
                        'IpProtocol': 'tcp',
                        'FromPort': 22,
